...
class Listofusers : AppCompatActivity() {
    ...
    fun afficher() {
        val url = "http://172.24.96.1/API%20PHP/Operations/CRUD/Read.php"
            ...
            try {
                if (res.getString("success").equals("1")) {
                    ...
                    for (i in 0 until dt.length()) {
                        lst.add(
                            User(
                                dt.getJSONObject(i).getInt("id_demande"),
                                dt.getJSONObject(i).getString("type_document"),
                                dt.getJSONObject(i).getString("date_demande"),
                                dt.getJSONObject(i).getString("statut"),
                                dt.getJSONObject(i).getString("date_saisie")
                            )
                        )
                        //alert("hello",lst.get(i).username)
                        lstview.adapter =
                            MyAdapter(this, R.layout.listingusers, lst as ArrayList<User>)
