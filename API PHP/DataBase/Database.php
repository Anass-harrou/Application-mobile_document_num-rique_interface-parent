<?php
    ...
    class Database{
        ...
        {
            $sql='SELECT * FROM parents WHERE username="'.$email.'"';
            ...
            $checkPass=$row['passwd'];


            if($checkPass == $password):
                    return $row["nom_parent"];
            endif;
            endif;
        ...
        public function SelectedByID($id){
            $sql='SELECT type_document,statut FROM assiste WHERE id_demande='.$id.' LIMIT 1';
            $stmt=$this->con->prepare($sql);
            $stmt->execute();
            if($stmt->rowCount()):
                $row=$stmt->fetch(PDO::FETCH_ASSOC);
        ...
        public function SelecteAll(){
            $sql='SELECT id_demande,type_document,date_demande,statut,date_saisie FROM assiste';
            $stmt=$this->con->prepare($sql);
            $stmt->execute();
            if($stmt->rowCount()):
                return $stmt;
